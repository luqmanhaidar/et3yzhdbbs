...
public class Information {
	...
	private void setRequestParameter(HttpServletRequest request)
		...
		if (!"Y".equals(this.sIfPic))
			this.sIfPic = "N";
		
		this.sAttm = request.getParameter("sattm");
		
		if(null==sAttm||sAttm.equals("")){
			this.sIfPic = "N";
		}
		this.auditUserId = iPubUserId;
		this.editUserId = iPubUserId;
		this.ding=Integer.parseInt(request.getParameter("ding"));
	...
	public boolean add(PageContext pageContext) {
		...
		try {
			...
			try {
				...
				String sql = "insert into j_information (IMODULEID, SINFOTITLE ,dpubdate, IPUBUSERID,"
						+ " SIFPIC, SATTM, ISOK, EDITUSERID, LASTEDITDATE,DING ) values (?,?,now(),?,?,?,?,?,now(),?)";
				...
				stmt.setString(6, "N");
				stmt.setInt(7, this.iPubUserId);
				stmt.setInt(8, this.ding);
		...
		try {
			...
			try {
				...
				String sql = "update j_information set IMODULEID=?, SINFOTITLE=?,"
						+ " SIFPIC=?, SATTM=?, ISOK='N', EDITUSERID=?, LASTEDITDATE=now(),ding=?  where IINFOID=?";
				PreparedStatement stmt = con.prepareStatement(sql);
				stmt.setInt(1, this.iModuleId);
				stmt.setString(2, this.sInfoTitle);
				...
				stmt.setInt(6, this.ding);
				stmt.setInt(7, this.iInfoId);
				stmt.setInt(5, this.editUserId);
				stmt.executeUpdate();
				stmt.close();

				int iSInfoLength = this.cinfo == null ? 0 : this.cinfo.length;
				for (int i = 0; i < iSInfoLength; i++) {
					if (Table.getRecordCount("j_information_pages", "IINFOID="
							+ this.iInfoId + " AND IPAGEID=" + (i + 1)) < 1) {
