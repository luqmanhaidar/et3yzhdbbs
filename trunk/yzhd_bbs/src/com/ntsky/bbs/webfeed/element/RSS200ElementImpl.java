...
public class RSS200ElementImpl extends RSSElementCommon implements RSSElement {

    private static Logger __logger = Logger.getLogger(RSS200ElementImpl.class
            .getName());
    ...
    public void feedRSS(Channel channel, String destXMLFile)
        ...
        Element root = domData.newRSSRoot(Globals.VERSION_RSS_200);
        ...
        if ((lastBuildDate != null)
                && !(Globals.EMPTY_STR.equals(lastBuildDate))) {
            domData.addTextElement(echannel, Globals.CHANNEL_LASTBUILDDATE,
                    lastBuildDate);
            lastBuildDate = null;
        }
    ...
    private void addItem(DOMData domData, Element parElement, Item item)
        ...
        if ((comments != null) && !(Globals.EMPTY_STR.equals(comments))) {
            domData.addCDATAElement(eitem, Globals.ITEM_COMMENTS, comments);
            comments = null;
        }        
        // author
        String author = item.getAuthor();
        if ((author != null) && !(Globals.EMPTY_STR.equals(author))) {
            domData.addTextElement(eitem, Globals.ITEM_AUTHOR, author);
            author = null;
        }            
        //pubDate
        String pubDate = item.getPubDate();
        if ((pubDate != null) && !(Globals.EMPTY_STR.equals(pubDate))) {
            domData.addTextElement(eitem, Globals.ITEM_PUBDATE, pubDate);
            pubDate = null;
        }          
