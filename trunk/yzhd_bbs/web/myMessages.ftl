      ...
      <div>
        ...
        <table align="center" cellpadding="0" class="ltable" cellspacing="1" style="margin:0px;width:100%;">
          ...
          <tr class="ltrv" style="background-color:<#if message.isRead=0>#f7f7f7;<#else>#fff;</#if>">
            ...
            <td width="8%"><a href="deleteMessage-receiver.action?messageId=${message.id}"><img src="images/delete.gif" alt="" width="45" height="18" /></a></td>
          </tr>
          </#list>
          </form>
          ...
          <tr class="ltrv">
            <td colspan="6" align="left" style="padding-left:12px;"><input type="checkbox" name="checkbox2" value="checkbox" onClick="if(this.checked){Forms.checkedAllBox('myMessages');}else{Forms.unCheckedAllBox('myMessages');}"/>
              选中全部的短消息<input type="button" value="删除选中的短消息" name="agree" class="b" onClick="System.batchExecute('myMessages','确认是否删除选中的消息','deleteMoreMessage-receiver.action');"/></td>
          </tr>		  
        </table>
      </div>
      <!-- explain begin -->
