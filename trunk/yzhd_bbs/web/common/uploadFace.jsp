...
<div class="box1">
  <div class="title"> 上传头像 </div>
	<form action="<%=request.getContextPath()%>/common/doUploadFace.jsp" method="post" onSubmit="return Validator.Validate(this,2)" enctype="multipart/form-data">
    <input type="hidden" value="<%=element%>" name="element"/>
	<input type="hidden" value="<%=preview%>" name="preview"/>
    <div class="content">
      <div class="ibox">
        <div class="it">选择头像: * </div>
        <div class="iv">
          <input name="file" type="file" class="t" value="" style="padding:0px; "/>
          <span class="red">选择上传的头像</span> </div>
      </div>
    </div>
    <!-- end #content -->
    <div class="box3" style="text-align:center; clear:both">
      <div>
        <input type="submit" value="上传头像" name="agree" class="b"/>
      </div>
    </div>
  </form>
</div>
