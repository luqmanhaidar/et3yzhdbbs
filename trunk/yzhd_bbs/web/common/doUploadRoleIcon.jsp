...
<%
	try{
		...
		iu.setSaveDirectory("skins/default/level");
		//iu.setFilePrefixName("20051101111111");
		iu.setMaxFileSize(1024);
...
%>
		<script>
			alert("成功上传角色标识图片!");
			if(opener!=null){
				opener.document.getElementById("<%=element%>").value = "<%out.print(file.getFileName());%>";
				if(!("false"=="<%=preview%>")){
					opener.document.images["view<%=element%>"].src = "<%					out.print(request.getContextPath()+"/skins/default/level/"+file.getFileName());%>";
				}
			}
			window.close();
		</script>
