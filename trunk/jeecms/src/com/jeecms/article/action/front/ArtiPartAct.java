...
public class ArtiPartAct extends CmsPartAction {
	private void tagArtiList() {
		...
		if (!StringUtils.isBlank(attr)) {
			ctgId = artiCtgMng.getArtiCtg(getWebId(), attr).getId();
		}
		pagination = articleMng.getForTag(getWebId(), chnlId, ctgId, searchKey,
				hasTitleImg, recommend == 1 ? true : false, orderBy, "1"
						.equals(isPage) ? true : false, firstResult, pageNo,
				count);
	}
