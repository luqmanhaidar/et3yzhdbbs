<#macro table cols normalOperate=[] batchOperate=[] wholeOptName="" wholeOptAction="" keepParams=[] gotoPageAction="Com_list"
  actionSuffix=".do" keyId="id" batchId="ids" wholeId="wids" value=pagination isPagination=true rowIndex=true operateCol="操作"
  width="100%">
...
<#if batchOperate?size gt 0>
	<th width="20px"><input type="checkbox" name="allCheck" value="checkbox" onclick="Pn.checkBox('${batchId}',this.checked);"/></th>
</#if>
<#if rowIndex>
	<th width="20px">#</th>
</#if>
...
<div class="pn-sp">
	...
	<div class="pn-sp-right">
		...
		<input type="button" value="上一页" onclick="_gotoPage('${value.nextPage}');"<#if value.firstPage> disabled="disabled"</#if>/>&nbsp;
		<input type="button" value="下一页" onclick="_gotoPage('${value.prePage}');"<#if value.lastPage> disabled="disabled"</#if>/>&nbsp;
		<input type="button" value="尾 页" onclick="_gotoPage('${value.totalPage}');"<#if value.lastPage> disabled="disabled"</#if>/> &nbsp;
		当前 ${value.pageNo}/${value.totalPage} 页 &nbsp;转到第 <input type="text" size="2" onfocus="this.select();" onkeypress="if(event.keyCode==13){$('#_goPage').click();return false;}"/> 页
