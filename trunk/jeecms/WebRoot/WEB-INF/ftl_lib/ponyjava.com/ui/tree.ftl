...
<#macro tree root treeId="t" target="rightFrame" showDeep=0 
	isCheckBox=false checkBoxName="ids" valueContent=[] 
	isLimited=false content=[] 
...
<@drawNode node=root treeId=treeId pid=treeId target=target showDeep=showDeep 
	...
	isLimited=isLimited content=content 
...
<#macro drawNode node pid treeId target showDeep isCheckBox checkBoxName valueContent 
	isLimited content url durl prefix suffix isUrl
...
<input type="checkbox" name="${checkBoxName}" <#rt/>
	<#if valueContent?seq_contains(node.id)>checked="true"</#if><#lt/>value="${node.id}" id="${id}-chk" class="pntree-checkbox" size="0" hidefocus="true" onclick="Pn.Tree.switchSelect(this,'${id}','${treeId}');" /><#t/>
</#if>
<#--名称列-->
<#if !isLeaf>
<#if !isCheckBox>
...
" target="${target}" onclick="Pn.Tree.switchDisplay('${id}',true)">${node.treeName}</a>
<#else>
<span onclick="Pn.Tree.switchDisplay('${id}')" style="cursor:pointer;">${node.treeName}</span>
</#if>
</div>
<#--子节点开始-->
...
<@drawNode node=cnode pid=id target=target treeId=treeId showDeep=showDeep 
	isCheckBox=isCheckBox checkBoxName=checkBoxName valueContent=valueContent 
	isLimited=isLimited content=content url=url durl=durl prefix=prefix suffix=suffix isUrl=isUrl 
